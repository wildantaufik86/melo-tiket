...
export default function UserFormModal({ isOpen, onClose, onSave, editingUser }: ModalProps) {
    ...
    useEffect(() => {
        if (editingUser) {
            ...
            setFormData({ name: '', email: '', idNumber: '', role: 'user', password: '' });
        }
    }, [editingUser, isOpen]);
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="idNumber" value={formData.idNumber} onChange={handleChange} className="w-full border p-2 rounded" required />
                    </div>
                    <div>
                        <label>Role</label>
                    ...
                    <div>
                        ...
                        <input type="password" name="password" value={formData.password} onChange={handleChange} className="w-full border p-2 rounded" placeholder={isEditMode ? "Leave blank to keep current password" : ""} required={!isEditMode} />
                    </div>
                    <div className="flex justify-end gap-4 pt-4 border-t">
                        <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
                        <button type="submit" disabled={submitting} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:bg-blue-400">
